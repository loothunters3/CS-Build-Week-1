...
#from .util import gen_room, Descriptor, change_coords
import random
import uuid

class World_Model(models.Model):
    users = models.ManyToManyField(User)
    uuid = models.UUIDField(default=uuid.uuid4, unique=True)
    limit = models.IntegerField(default=501)
    seed = models.TextField(default=None, null=True)
    coords = JSONField(default=None, null=True, blank=True)
    ...
    def generate_room(self, curr_room, direction):
        ...
        self.coords[str(targ_coords)] = new_room.id
...
class Room(models.Model):
    ...
    world = models.ForeignKey(World_Model, on_delete=models.CASCADE, default=None, null=True)
...
class Player(models.Model):
    ...
    uuid = models.UUIDField(default=uuid.uuid4, unique=True)
    currentWorld = models.OneToOneField(World_Model, on_delete=models.CASCADE, default=None, null=True)
